...
############################## Choice Model
class ChoiceModelUtility(nn.Module):
    def __init__(self, data, H, device):
        super(ChoiceModelUtility, self).__init__()
        self.data = data
        self.device = device
        ...
        self.U_SUSP_SEX = nn.Parameter(torch.empty(len(self.SUSP_SEX_list), self.M).uniform_(1.00, 1.00))
        # self.U_SUSP_RACE = nn.Parameter(torch.empty(len(self.SUSP_RACE_list), self.M).uniform_(1.00, 1.00))
        
        # self.pi = nn.Parameter(torch.randn(self.H)) # .to(self.device)
        
        self.pi_gender = nn.Parameter(torch.randn(self.H, len(self.SUSP_SEX_list)))
    ...
    def prob_loc_time_pair(self, g_res_H):   
        ...
        pi_gender_clone = self.pi_gender.clone()
        
        # normalized_pi = F.softmax(pi_clone, dim=0)
        normalized_pi_gender = F.softmax(pi_gender_clone, dim=0)     
                
        # normalized_pi = normalized_pi.unsqueeze(1).repeat(1, self.M)
        # normalized_pi = normalized_pi.unsqueeze(1).repeat(1, len(self.SUSP_SEX_list), 1)
        normalized_pi_gender = normalized_pi_gender.unsqueeze(2).repeat(1, 1, self.M)
    
        U = self.U_SUSP_SEX.unsqueeze(0).repeat(self.H, 1, 1)
        # U = self.U_SUSP_RACE.unsqueeze(0).repeat(self.H, 1, 1)   
        
        # normalized_pi = normalized_pi.unsqueeze(1).repeat(1, len(self.SUSP_RACE_list), 1)
        g_res_H = g_res_H.unsqueeze(1).repeat(1, len(self.SUSP_SEX_list), 1)
        # g_res_H = g_res_H.unsqueeze(1).repeat(1, len(self.SUSP_RACE_list), 1)

        upper = torch.mul(g_res_H, torch.exp(U))
        upper = self.relu(upper) # to avoid numerical issue

        lower = torch.sum(torch.mul(g_res_H, torch.exp(U)), dim=2)

        lower = lower.unsqueeze(2).repeat(1, 1, self.M)

        # P_M = torch.sum(normalized_pi * (upper / lower), dim=0)
        P_M = torch.sum(normalized_pi_gender * (upper / lower), dim=0)

        return P_M
        
    def log_likelihood(self, time, precinct, race, sex):
        ...
        event_one_hot = torch.zeros(batch_size, self.M).to(self.device)    

        indices_tensor = torch.LongTensor(indices).to(self.device)
        event_one_hot.scatter_(1, indices_tensor.unsqueeze(1), 1)
        
        ##### gender
        sex_F_one_hot = torch.zeros_like(sex).to(self.device)
        sex_M_one_hot = torch.zeros_like(sex).to(self.device)
        sex_F_one_hot[sex == 0] = 1
        sex_M_one_hot = sex
        sex_one_hot = torch.stack([sex_F_one_hot, sex_M_one_hot])
        sex_one_hot = sex_one_hot.unsqueeze(2).repeat(1, 1, self.M)
        ...
        event_one_hot = event_one_hot.unsqueeze(0).repeat(len(self.SUSP_SEX_list), 1, 1)
        # event_one_hot = event_one_hot.unsqueeze(0).repeat(len(self.SUSP_RACE_list), 1, 1)

        P_M = P_M.unsqueeze(1).repeat(1, batch_size, 1)  
        
        temp = P_M * event_one_hot * sex_one_hot
        # temp = P_M * event_one_hot * race_one_hot
        
        batch_ll = torch.log(torch.sum(torch.sum(temp, dim=0), dim=1))
