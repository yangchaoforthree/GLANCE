...
class EntmaxBisectFunction(Function):
    @classmethod ## 通过@classmethod定义的类方法可通过类名调用，而不需要先创建类的实例
    def _gp(cls, x, alpha):
        return x ** (alpha - 1)
    ...
    def forward(cls, ctx, X, alpha=1.5, dim=-1, n_iter=50, ensure_sum_one=True):
        ...
        if ensure_sum_one:
            # print('p_m.sum(dim=dim).unsqueeze(dim=dim)', p_m.sum(dim=dim).unsqueeze(dim=dim))
...
class SparsemaxBisectFunction(EntmaxBisectFunction):
    ## 在 Python 中，@classmethod 装饰器用于定义一个类方法，即一个可以被类本身调用而不是实例调用的方法。
    ## 类方法的第一个参数通常被命名为 cls，它表示类本身，而不是实例对象。这个参数在类方法内部通常用来访问类的属性或调用其他类方法。
    @classmethod
    def _gp(cls, x, alpha):
        return x
